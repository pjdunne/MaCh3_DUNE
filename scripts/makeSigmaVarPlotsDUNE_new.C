...
void makeSigmaVarPlotsDUNE_new(TString inputfile)
{
  ...
  gStyle->SetOptStat(0);
   ...
   std::vector<std::string> plotvariables = {"NChargedPions", "TrueNeutrinoEnergy", "PiTrueEnergy", "PiTrueMinEnergy"};

for(int iplotvars = 0; iplotvars<plotvariables.size(); iplotvars++){
   for(int i=0; i<list->GetEntries(); i++)
   {
//     std::cout << "entry " << i << std::endl;
     TString keyname = list->At(i)->GetName();
     if(keyname.Contains(plotvariables[iplotvars].c_str())){
	 ...
	 //for(int j=0; j<4; j++)
      ...
      if(keyname.Contains("NDGAr_FHC")) {
       std::string namefull = "NDGAr_FHC"+plotvariables[iplotvars]+"_unosc";
       TH1D* ndgar_fhc_ccnumu_nom = (TH1D*)file->Get(namefull.c_str());
       ndgar_fhc_ccnumu_nom->SetLineColor(kBlack);
       ndgar_fhc_ccnumu_nom->GetXaxis()->SetTitle(plotvariables[iplotvars].c_str());
...
//      maxlist.push_back(divisor->GetMaximum());
      for(int j=0; j<4; j++) {
         maxlist.push_back(hVar[j]->GetMaximum()); std::cout<<hVar[j]->GetMaximum()<<std::endl; }
      std::cout<<"maxlist.size(): "<<maxlist.size()<<std::endl;
      std::cout<<"maxlist[0]: "<<maxlist[0]<<std::endl;
      std::cout<<"max element: "<<*std::max_element(maxlist.begin(), maxlist.end())<<std::endl;
      double maxmax = (*std::max_element(maxlist.begin(), maxlist.end()));
//      double maxmax = *maxmaxit;
