...
// Changed Binning for 1sig only
  ...
  h_dcp_th23_stat = new TH2D("h_dcp_th23_DUNE", "DUNE FD ;sin^{2}#theta_{23};#delta_{CP}", hh->GetNbinsY(), hh->GetYaxis()->GetXmin(), hh->GetYaxis()->GetXmax(), hh->GetNbinsX(), hh->GetXaxis()->GetXmin()*TMath::Pi(), hh->GetXaxis()->GetXmax()*TMath::Pi());
  ...
  for (int i = 0; i < hh->GetNbinsX(); i++)
  {
	for (int j = 0; j < hh->GetNbinsY(); j++)
	{
      ...
      h_dcp_th23_stat->SetBinContent(j , i, log);
	}
  }

  h_dcp_th23_stat->Rebin2D();
  */
  ...
  h_dm32_th23_asimov->GetYaxis()->SetLabelSize(0.07); 
  ...
  if(app == 1 && hierarchy==0) leg = new TLegend(0.55,0.53,0.80,0.85);
  else if(app == 1 && hierarchy!=0) leg = new TLegend(0.55,0.55,0.80,0.85);
  else leg = new TLegend(0.55,0.55,0.80,0.85);
  ...
  if(!levSigs) {
    ...
    leg->AddEntry(bestfitM_stat,"Stat-only b.f","p");
    ...
    leg->AddEntry(h_dcp_th13_cont_2sig_stat, stat_title, "l");
    leg->AddEntry(bestfitM_stat,"DUNE TDR b.f","p");
	//stat-only
