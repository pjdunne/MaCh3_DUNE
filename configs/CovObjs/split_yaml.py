...
def get_from_full_yaml(out_name,
    ...
    with open("tdr_covs_grouped.yaml", 'r') as stream:
        try:
...
if __name__ == '__main__':
    ...
    for det_id in det_ids:
        ...
        for param_group in param_groups:
            get_from_full_yaml(f"tdr_covs/{param_group}_{det_id_name}.yaml",
                                det_id=[det_id],
                                syst_type=syst_types,
                                horn_current=horn_currents,
                                param_groups=[param_group])
            get_from_full_yaml(f"tdr_covs/{param_group}_{det_id_name}_uniform.yaml",
                                det_id=[det_id],
                                syst_type=syst_types,
                                horn_current=horn_currents,
                                param_groups=[param_group],
    ...
    get_from_full_yaml('tdr_covs/Flux_ND+FD.yaml',
                          det_id=[1,24],
                          syst_type=syst_types,
                          horn_current=['fhc','rhc'],
                          param_groups=['Flux'])
    get_from_full_yaml('tdr_covs/Flux_ND+FD_uniform.yaml',
                       det_id=[1,24],
                          syst_type=syst_types,
                            horn_current=['fhc','rhc'],
                            param_groups=['Flux'],
